export const geminiBionic = async (text) => {
  try {
    ...
    const response = await fetch(GEMINI_API_URL, {
      ...
      body: JSON.stringify({
        system_instruction: {
          parts: [
            {
              text: "Convert the following text into Bionic Reading format. Bold the first few letters of each word for easier readability:"
            }
          ],
        },
        contents: {
    ...
    return result.candidates && result.candidates[0]?.content 
      ? result.candidates[0]?.content 
      : 'Error in converting text.';
